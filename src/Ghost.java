...
public class Ghost {
    ...
    public void move(Tile[][] board, Player player) {
        if (section == numSections / 2) {
            if (board[y][x] == Tile.JUNCTION) {
    ...
    private Direction[] getValidSpecialDirections(Tile[][] board) {
        List<Direction> directions = new ArrayList<>();
        for (int i = 0; i < Direction.values().length; i++) {
            if (Direction.values()[i] == DirectionHelp.getOppositeDirection(direction) || Direction.values()[i] == Direction.UP) {
                continue;
            }
            Tile t = tryGetTile(board, Direction.values()[i]);
            if (t != Tile.WALL && t != null) {
    ...
    private int getDistance(Direction d, Player player) {
